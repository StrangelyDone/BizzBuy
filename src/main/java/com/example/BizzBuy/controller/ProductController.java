...
public class ProductController {
    ...
    public ResponseEntity<?> createProduct(@RequestHeader("X-USER") String username, @RequestBody Product product) {
        ...
        // errors from the user side..!
            ...
            // map cuz its the closest to JSON we got in java..! json cuz its cleaner than
            // plain text..!
    ...
    @GetMapping("/filter")
    public ResponseEntity<?> filterProducts(@RequestParam(required = false) Double min,
            @RequestParam(required = false) Double max, @RequestParam(required = false) Long sellerId) {
        try {
    ...
    // mapping for the endpoint: (GET,
    // {{base_url}}/api/items/{{seller_id}}/{{product_id}})
    // ###############################oke nvm, no longer in
    // use..!########################3
    ...
    // mapping for the endpoint: (PUT,
    // {{base_url}}/api/items/{{product_id}}/stock?stock={{number}})
    @PutMapping("/{id}/stock")
    public ResponseEntity<?> updateStock(@RequestHeader("X-USER") String username, @PathVariable Long id,
    ...
    // DELETE /api/items/products/{id} - Delete product (Seller only)
    @DeleteMapping("/products/{id}")
    public ResponseEntity<?> deleteProduct(@RequestHeader("X-USER") String username, @PathVariable Long id) {
        try {
            User current = userService.requireByUsername(username);
            productService.deleteProduct(id, current.getId());
